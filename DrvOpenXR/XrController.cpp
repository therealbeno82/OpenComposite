...
{
	// Games request controller poses ahead of the frame's display time (that's what OpenVR's
	// GetDeviceToAbsoluteTrackingPose prediction is for); the convention is that the prediction is
	// relative to the display time, so we relocate the pose by the same offset the backend computed.
	// The hand-tracking fallback (GetPoseFromHandTracking) ignores the offset, which is fine: it's
	// only used when the controller itself is untracked.
	...
	// The session (and xr_gbl with it) can be briefly torn down and rebuilt by another thread; the HMD
	// path waits for it to come back, but a controller pose is only ever best-effort anyway.
	if (!xr_gbl)
		return;
	...
	if (xr_utils::PoseFromSpace(pose, space, origin, transform, timeOffsetNs)) {
