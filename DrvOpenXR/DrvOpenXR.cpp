...
#include <chrono>
#include <cinttypes> // for PRIu64 in the startup trace logging
#include <memory>
#include <set>
#include <string>
#include <thread>
...
#ifdef _WIN32
// For PathStripPathA, used by GetExeName below
#include <shlwapi.h>
...
static XrResult TryCreateSystemID()
{
	// Create a system - this is when we choose what form factor we want, in this case an HMD
	XrSystemGetInfo systemInfo{};
	systemInfo.type = XR_TYPE_SYSTEM_GET_INFO;
	systemInfo.formFactor = XR_FORM_FACTOR_HEAD_MOUNTED_DISPLAY;
	return xrGetSystem(xr_instance, &systemInfo, &xr_system);
}

static void CreateSystemID()
{
	// XR_ERROR_FORM_FACTOR_UNAVAILABLE means "I support HMDs, but I haven't got one for you right
	// now". With a streaming runtime that's routinely a transient condition rather than a fatal
	// one: the headset link drops for a moment, the runtime hasn't finished re-attaching after we
	// tore a session down, or the game was launched a beat before the headset finished connecting.
	// Aborting on the first try turns all of those into a crash dialog, so give it a couple of
	// seconds to come back before giving up.
	constexpr int maxAttempts = 20;
	constexpr int retryDelayMs = 100;

	XrResult res = XR_SUCCESS;
	for (int attempt = 0; attempt < maxAttempts; attempt++) {
		res = TryCreateSystemID();
		if (res != XR_ERROR_FORM_FACTOR_UNAVAILABLE)
			break;

		if (attempt == 0)
			OOVR_LOGF("xrGetSystem: no HMD available yet, retrying for up to %dms...", maxAttempts * retryDelayMs);

#ifdef _WIN32
		Sleep(retryDelayMs);
#else
		struct timespec ts = { 0, retryDelayMs * 1000000 };
		nanosleep(&ts, &ts);
#endif
	}
...
IBackend* DrvOpenXR::CreateOpenXRBackend(const char* startupInfo)
{
	...
	// STARTUP TRACE: log exactly which extensions we're asking the runtime to enable. If instance
	// creation dies, the culprit is almost always one of these.
	OOVR_LOGF("[startup] Requesting %zu extensions (apiFlags=0x%x):", extensions.size(), (unsigned)apiFlags);
	for (const char* ext : extensions)
		OOVR_LOGF("[startup]   enable: %s", ext);
	...
	OOVR_LOG("[startup] Calling xrCreateInstance...");
	...
	OOVR_LOG("[startup] xrCreateInstance OK");

	// Log what runtime we actually got - useful for bug reports, and confirms the loader picked the
	// runtime we expected rather than something else.
	{
		XrInstanceProperties instProps{ XR_TYPE_INSTANCE_PROPERTIES };
		if (XR_SUCCEEDED(xrGetInstanceProperties(xr_instance, &instProps))) {
			OOVR_LOGF("[startup] Runtime: '%s' version %u.%u.%u",
			    instProps.runtimeName,
			    (unsigned)XR_VERSION_MAJOR(instProps.runtimeVersion),
			    (unsigned)XR_VERSION_MINOR(instProps.runtimeVersion),
			    (unsigned)XR_VERSION_PATCH(instProps.runtimeVersion));
		}
	}
	...
	OOVR_LOG("[startup] Binding extension function pointers (XrExt)...");
	xr_ext = new XrExt(apiFlags, extensions);
	OOVR_LOG("[startup] XrExt OK");

	OOVR_LOG("[startup] Calling xrGetSystem...");
	CreateSystemID();
	OOVR_LOGF("[startup] xrGetSystem OK (xr_system=%" PRIu64 ")", (uint64_t)xr_system);

	// List off the views and store them locally for easy access
	OOVR_LOG("[startup] Enumerating view configuration views...");
	...
	OOVR_LOGF("[startup] Got %u views, recommended size %ux%u, max swapchain samples %u",
	    viewCount,
	    viewCount > 0 ? xr_views_list[0].recommendedImageRectWidth : 0,
	    viewCount > 0 ? xr_views_list[0].recommendedImageRectHeight : 0,
	    viewCount > 0 ? xr_views_list[0].maxSwapchainSampleCount : 0);
...
#error No available temporary graphics implementation
	...
	OOVR_LOGF("[startup] Creating XrBackend (temporary graphics: vulkan=%d d3d11=%d)...",
	    (int)useVulkanTmpGfx, (int)useD3D11TmpGfx);
	currentBackend = new XrBackend(useVulkanTmpGfx, useD3D11TmpGfx);
	OOVR_LOG("[startup] XrBackend OK");

	// Setup our OpenXR session
	OOVR_LOG("[startup] Calling SetupSession...");
	SetupSession();
	OOVR_LOG("[startup] SetupSession OK");

	handleStartupInfo(startupInfo);
	OOVR_LOG("[startup] Init complete - handing control back to the game");
...
void DrvOpenXR::SetupSession()
{
	...
	OOVR_LOG("[startup] SetupSession: fetching graphics binding...");
	sessionInfo.next = currentBackend->GetCurrentGraphicsBinding();
	OOVR_LOGF("[startup] SetupSession: graphics binding type=%d, calling xrCreateSession...",
	    sessionInfo.next ? (int)((const XrBaseInStructure*)sessionInfo.next)->type : -1);
	OOVR_FAILED_XR_ABORT(xrCreateSession(xr_instance, &sessionInfo, &xr_session.get()));
	OOVR_LOG("[startup] SetupSession: xrCreateSession OK");

	// Setup the OpenXR globals, which uses the current session so we have to do this last
	OOVR_LOG("[startup] SetupSession: creating reference spaces (XrSessionGlobals)...");
	xr_gbl = new XrSessionGlobals();
	OOVR_LOG("[startup] SetupSession: XrSessionGlobals OK");
	...
	if (input) {
		OOVR_LOG("[startup] SetupSession: rebinding inputs for new session...");
		input->BindInputsForSession();
		OOVR_LOG("[startup] SetupSession: inputs bound");
	}

	OOVR_LOG("[startup] SetupSession: waiting for session state transition (OnSessionCreated)...");
	currentBackend->OnSessionCreated();
	OOVR_LOG("[startup] SetupSession: OnSessionCreated OK");
}

void DrvOpenXR::ShutdownSession(bool forRestart)
{
	...
	// Everything from here on is best-effort. We're tearing the session down either way, and by far
	// the most common reason for these calls to fail is that the runtime has already dropped the
	// session out from under us - which is exactly the case where turning it into an abort (and so
	// a crash dialog) is least helpful.
	if (currentBackend->sessionActive) {
		// Hey it turns out that xrDestroySession can be called whenever - how convenient
		XrResult exitRes = xrRequestExitSession(xr_session.get());
		if (XR_FAILED(exitRes)) {
			OOVR_LOGF("xrRequestExitSession failed (%d) - session is probably already gone, continuing shutdown", exitRes);
		} else {
			int count = 0;
	...
	XrResult destroyRes = xrDestroySession(xr_session.get());
	if (XR_FAILED(destroyRes))
		OOVR_LOGF("xrDestroySession failed (%d), continuing shutdown anyway", destroyRes);
	xr_session.reset();

	// Only worth doing if a new session is about to be created - see the header doc. On the way out
	// this used to abort the game with an "OpenComposite Error" dialog whenever the headset had
	// already disconnected, since xrGetSystem then returns XR_ERROR_FORM_FACTOR_UNAVAILABLE. A
	// headset that's gone away is the normal reason to be shutting down, not an error to report.
	if (forRestart)
		CreateSystemID();
}
...
void DrvOpenXR::FullShutdown()
{
	...
	if (xr_instance) {
		// Best-effort for the same reason as the session teardown above: we're exiting regardless,
		// and there's nothing useful the user can do about a failure here.
		XrResult res = xrDestroyInstance(xr_instance);
		if (XR_FAILED(res))
			OOVR_LOGF("xrDestroyInstance failed (%d), continuing shutdown anyway", res);
		xr_instance = XR_NULL_HANDLE;
	}

	// The extension function pointers belong to the instance we just destroyed
	delete xr_ext;
	xr_ext = nullptr;
