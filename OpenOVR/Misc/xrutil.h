...
public:
	 ...
	 *
	 * Note this deliberately returns by value rather than by reference: ClearCachedViews is called from the
	 * render thread while other threads may be reading views, so handing out references into the map would
	 * be a use-after-free waiting to happen. The struct is small enough that the copy doesn't matter.
	 */
	XruCachedViews GetCachedViews(XrSpace space);

	/**
	 * Discard all cached xrLocateViews results, so the next GetCachedViews call re-queries the runtime.
	 */
