...
bool xr_utils::PoseFromSpace(vr::TrackedDevicePose_t* pose, XrSpace space, vr::ETrackingUniverseOrigin origin,
    std::optional<glm::mat4> extraTransform, int64_t timeOffsetNs)
{
	...
	// timeOffsetNs offsets the locate time from the frame's display time - positive values look into
	// the future (prediction), negative into the past. XrTime has no absolute wall-clock mapping, so
	// the offset is relative to GetBestTime() rather than an absolute timestamp.
	OOVR_FAILED_XR_SOFT_ABORT(xrLocateSpace(space, baseSpace, xr_gbl->GetBestTime() + timeOffsetNs, &info));
