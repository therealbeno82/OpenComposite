...
XruCachedViews XrSessionGlobals::GetCachedViews(XrSpace space)
{
	...
	OOVR_FAILED_XR_SOFT_ABORT(locateResult = xrLocateViews(xr_session.get(), &locateInfo, &viewState, XruEyeCount, &viewCount, views));

	// Don't cache a failed lookup. Callers such as XrHMD::GetProjectionMatrix hard-abort when viewCount
	// isn't XruEyeCount, so latching a transient runtime failure here until the next ClearCachedViews
	// would turn a recoverable hiccup into a killed game.
	if (XR_FAILED(locateResult) || viewCount != XruEyeCount) {
		XruCachedViews failed{};
		failed.viewState = viewState;
		failed.viewCount = viewCount;
		std::copy(views, views + XruEyeCount, failed.views.begin());
		return failed;
	}
