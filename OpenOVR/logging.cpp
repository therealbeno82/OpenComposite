...
/**
 * Guards all access to `stream`, including opening it.
 *
 * std::ofstream is not thread-safe for concurrent writes: racing calls to operator<< corrupt the
 * streambuf's heap-allocated buffer, which shows up later as heap corruption (STATUS_HEAP_CORRUPTION
 * / access violations inside ntdll) far away from the actual logging code. OpenComposite logs from
 * the game's render thread, its logic thread and our own, so this is very much reachable.
 *
 * Recursive because the abort macros log, and an abort raised while we're inside a log call would
 * otherwise deadlock against ourselves.
 */
...
static void init_stream()
{
	if (!stream.is_open()) {
		...
		// Keep the previous run's log around as opencomposite.log.1. Opening the stream truncates,
		// so without this the log for a failed run is destroyed the moment anything VR-related is
		// launched again - including a second attempt at the same game. Intermittent problems are
		// exactly the ones where you need the run before last to compare against.
		{
			const std::string previousPath = outputFilePath + ".1";
			std::remove(previousPath.c_str());
			std::rename(outputFilePath.c_str(), previousPath.c_str());
		}
...
#else
	// See the comment on log_mutex - this guards both the lazy open and the write.
