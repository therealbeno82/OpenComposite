...
ovr_enum_t BaseCompositor::Submit(EVREye eye, const Texture_t* texture, const VRTextureBounds_t* bounds, EVRSubmitFlags submitFlags)
{
	...
	// CheckOrInitCompositors can end up restarting the session (which takes the exclusive lock), so it
	// must run BEFORE we take the shared lock here - taking it first would make the restart a no-op due
	// to thread-local reentrancy, and then the swapchain teardown could race the copy below.
	// This also protects the graphics-API-switch restart: the shared lock holds off teardown until the
	// frame is fully submitted.
	if (!textureNull) {
		auto* backend = (XrBackend*)BackendManager::Instance().GetBackendInstance();
		if (backend)
			backend->CheckOrInitCompositors(texture);
	}
