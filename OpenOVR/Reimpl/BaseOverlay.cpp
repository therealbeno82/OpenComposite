...
EVROverlayError BaseOverlay::CreateSubviewOverlay(VROverlayHandle_t parentOverlayHandle, const char* pchSubviewOverlayKey,
    const char* pchSubviewOverlayName, VROverlayHandle_t* pSubviewOverlayHandle)
{
	// Subview overlays composite a child image onto a parent overlay. We'd have to build the composited
	// result ourselves since OpenXR has no equivalent, so report failure rather than silently handing back
	// a handle that never renders.
	OOVR_SOFT_ABORT("Subview overlays are not implemented");
...
{
	// Nothing can hold a valid subview handle, since CreateSubviewOverlay always fails.
	OOVR_SOFT_ABORT("Subview overlays are not implemented");
	return VROverlayError_InvalidHandle;
}
...
{
	// Validate the handle BEFORE dereferencing it - a destroyed overlay has been freed, and reading
	// its key is a use-after-free.
	OverlayData* overlay = (OverlayData*)ulOverlayHandle;
	if (!overlay || !validOverlays.count(overlay) || !overlays.count(overlay->key)) {
		if (pError)
	...
	if (unBufferSize != 0) {
		strncpy_s(pchValue, unBufferSize, key, _TRUNCATE);
	...
	return static_cast<uint32_t>(strlen(key) + 1);
}
uint32_t BaseOverlay::GetOverlayName(VROverlayHandle_t ulOverlayHandle, VR_OUT_STRING() char* pchValue, uint32_t unBufferSize, EVROverlayError* pError)
{
	...
	// A zero buffer size is a legitimate "what's the required size?" probe - report it without writing.
	if (unBufferSize != 0) {
		strncpy_s(pchValue, unBufferSize, name, _TRUNCATE);
		pchValue[unBufferSize - 1] = 0;
	}

	// Is this supposed to include the NULL or not?
	// TODO test, this could cause some very nasty bugs
	return static_cast<uint32_t>(strlen(name) + 1);
}
...
EVROverlayError BaseOverlay::SetOverlayTexture(VROverlayHandle_t ulOverlayHandle, const Texture_t* pTexture)
{
	...
	// The overlay texture is copied into the session's swapchain, so the session must not be torn
	// down underneath us (it can be, for a graphics-API switch or an input restart) - same protection
	// as BaseCompositor::Submit.
	{
...
uint32_t BaseOverlay::GetKeyboardText(char* pchText, uint32_t cchText)
{
	...
	if (cchText != 0) {
		...
		strncpy_s(pchText, cchText, str.c_str(), _TRUNCATE);
	...
	return (uint32_t)strlen(str.c_str());
}
