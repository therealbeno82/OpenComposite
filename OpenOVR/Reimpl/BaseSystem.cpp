...
{
	// We don't route input through overlays, so no event we generate is ever targeted at one. Report the
	// invalid handle and otherwise behave exactly like PollNextEventWithPose.
	if (pulOverlayHandle)
