...
EVRApplicationError BaseApplications::LaunchInternalProcess(const char* pchBinaryPath, const char* pchArguments, const char* pchWorkingDirectory)
{
	...
	// Constructing a std::string from a NULL pointer is UB; games may legitimately pass NULL args.
	string cmd = "\"" + string(pchBinaryPath ? pchBinaryPath : "") + "\" " + string(pchArguments ? pchArguments : "");
