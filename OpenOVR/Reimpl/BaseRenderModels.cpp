    ...
    char* pchComponentName, uint32_t unComponentNameLen)
{
	...
	if (pchComponentName) {
		...
		// Truncate rather than abort on undersized buffers - the OpenVR contract is that this
		// returns the required size regardless.
		strncpy_s(pchComponentName, unComponentNameLen, name.c_str(), _TRUNCATE);
    ...
    char* componentModelName, uint32_t componentModelNameLen)
{
	...
	if (componentModelName) {
		if (componentModelNameLen == 0)
			return (uint32_t)name.length() + 1;

		// Truncate rather than abort on undersized buffers - the OpenVR contract is that this
		// returns the required size regardless.
		strncpy_s(componentModelName, componentModelNameLen, name.c_str(), _TRUNCATE);
