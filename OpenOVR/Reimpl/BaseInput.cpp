...
static std::string escapePathString(const std::string& str)
{
	...
	for (char c : str) {
		...
		// Cast through unsigned char: a plain (signed) char >= 0x80 sign-extends to 0xffffffXX, and the
		// truncation to two characters would then collapse every high byte to "ff", making distinct
		// non-ASCII action names escape to the same OpenXR path.
		char hex[3];
...
EVRInputError BaseInput::SetActionManifestPath(const char* pchActionManifestPath)
{
	...
	if (hasLoadedActions) {
		...
		// The action sets are still attached to the live session here, so destroying them would fail with
		// XR_ERROR_ACTIONSET_ATTACHED. That's fine - the session restart below destroys the session (and
		// its attachments) anyway, and BindInputsForSession recreates fresh sets on the new one.
		for (std::unique_ptr<ActionSet>& as : actionSets.GetItems()) {
			XrResult res = xrDestroyActionSet(as->xr);
			if (XR_FAILED(res))
				OOVR_LOGF("xrDestroyActionSet for '%s' failed (%d) - it is still attached to the old session, continuing", as->name.c_str(), res);
		}
		XrResult legacyRes = xrDestroyActionSet(legacyInputsSet);
		if (XR_FAILED(legacyRes))
			OOVR_LOGF("xrDestroyActionSet for legacy inputs failed (%d) - it is still attached to the old session, continuing", legacyRes);
    ...
    uint32_t unSizeOfVRSelectedActionSet_t, uint32_t unSetCount)
{
	...
	if (unSetCount > 1) {
		...
		for (uint32_t i = 1; i < unSetCount; i++) {
			if (pSets[i].nPriority != priority) {
				...
				ActionSet* curAs = cast_ASH(pSets[i].ulActionSet);
				OOVR_SOFT_ABORTF("Active action set %s (%d) and %s (%d) have different priorities, this is not yet supported",
				    as1->fullName.c_str(), pSets[0].nPriority, curAs->fullName.c_str(), pSets[i].nPriority);
			}
		}
	}
	...
	for (uint32_t i = 0; i < unSetCount; i++) {
		...
		if (set.ulRestrictedToDevice != vr::k_ulInvalidInputValueHandle) {
			...
			// legacyControllers only has entries for the two hands - any other device type
			// (HMD, generic tracker) would index out of bounds.
			if (dev && (dev->GetHand() == ITrackedDevice::HAND_LEFT || dev->GetHand() == ITrackedDevice::HAND_RIGHT)) {
...
{
	// Single lookup - this runs for every action, every subaction path, every frame. Note that using
	// operator[] here would also insert a default entry into the map on a miss, which is a write on what
	// is otherwise a read-only path.
	const auto perProfileIter = action.perProfileData.find(profile);
	const bool hasPerProfileData = perProfileIter != action.perProfileData.end();
	const ActionPerProfileData& perProfileData = hasPerProfileData ? perProfileIter->second : Action::defaultActionData;
	...
	if (!perProfileData.dclickBindings.empty())
		for (auto& dclick_info : perProfileIter->second.dclickBindings) {
	...
	if (!perProfileData.dpadBindings.empty())
		for (auto& [parent_name, dpad_info] : perProfileIter->second.dpadBindings) {
    ...
    VRInputValueHandle_t ulRestrictToDevice)
{
	...
	for (size_t i = 0; i < allSubactionPaths.size(); i++) {
		...
		// Single lookup - see the note in getBooleanOrDpadData
		const auto profileIter = act->perProfileData.find(profile);
		const ActionPerProfileData& action_per_profile_data = profileIter != act->perProfileData.end()
		    ? profileIter->second
...
EVRInputError BaseInput::GetSkeletalReferenceTransforms(VRActionHandle_t actionHandle, EVRSkeletalTransformSpace eTransformSpace, EVRSkeletalReferencePose eReferencePose, VR_ARRAY_COUNT(unTransformArrayCount) VRBoneTransform_t* pTransformArray, uint32_t unTransformArrayCount)
{
	...
	// The reference poses are always a full BoneArray - refuse to write more than the caller gave us room
	// for, rather than overflowing their buffer.
	if (unTransformArrayCount < static_cast<uint32_t>(eBone_Count)) {
		OOVR_LOGF("WARNING: GetSkeletalReferenceTransforms called with %u transforms, need %d - refusing",
		    unTransformArrayCount, (int)eBone_Count);
    ...
    EVRSkeletalMotionRange eMotionRange, VR_ARRAY_COUNT(unTransformArrayCount) VRBoneTransform_t* pTransformArray, uint32_t unTransformArrayCount)
{
	...
	// There may be no profile at all when we're tracking hands without controllers (the usingFakePose case
	// above), in which case the pose is already relative to the floor and needs no grip correction.
	glm::mat4 transform = profile ? profile->GetGripToSteamVRTransform(hand) : glm::identity<glm::mat4>();
    ...
    uint32_t unBindingInfoSize, uint32_t unBindingInfoCount, uint32_t* punReturnedBindingInfoCount)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		case ActionType::Boolean:
			...
			strcpy_arr(info.rchInputSourceType, "button");
			break;
		case ActionType::Vector1:
...
void BaseInput::GetHandSpace(ITrackedDevice::TrackedDeviceType hand, XrSpace& space, bool aimPose)
{
	...
	// legacyControllers only has entries for the two hands - HAND_NONE (the HMD and generic trackers)
