...
found:
	...
	// +1 for the null - if the caller's buffer is too small, truncate rather than abort the process
	// (the caller asked for a string, getting a truncated one is far friendlier than a crash dialog).
	if (unValueLen == 0)
		return;

	strncpy_s(pchValue, unValueLen, result.c_str(), _TRUNCATE);
