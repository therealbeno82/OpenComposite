...
ID3DBlob* d3d_compile_shader(const char* hlsl, const char* entrypoint, const char* target)
{
	...
	if (FAILED(D3DCompile(hlsl, strlen(hlsl), nullptr, nullptr, nullptr, entrypoint, target, flags, 0, &compiled, &errors))) {
		// errors is null if the call failed for a reason other than a compile error (eg E_INVALIDARG),
		// so don't dereference it unconditionally.
		OOVR_ABORTF("Error: D3DCompile failed: %s", errors ? (const char*)errors->GetBufferPointer() : "<no compiler output>");
	}
	if (errors)
		errors->Release();
...
DX11Compositor::DX11Compositor(ID3D11Texture2D* initial)
{
	...
	fs_vert_shader_blob->Release();
	fs_pixel_shader_blob->Release();
...
DX11Compositor::~DX11Compositor()
{
	...
	if (quad_texture_view)
		quad_texture_view->Release();
	if (quad_sampleState)
		quad_sampleState->Release();
	if (fs_vshader)
		fs_vshader->Release();
	if (fs_pshader)
		fs_pshader->Release();
...
void DX11Compositor::CheckCreateSwapChain(const vr::Texture_t* texture, const vr::VRTextureBounds_t* bounds, bool cube)
{
	...
	// Keep the un-scaled description around: the swapchain gets sized to the bounded sub-rect, but the
	// MSAA resolve target must match the SOURCE texture, since ResolveSubresource resolves the whole
	// resource and the copy below then reads the bounded region out of it.
	const D3D11_TEXTURE2D_DESC fullSrcDesc = srcDesc;
	...
	if (!usable) {
		...
		// The app's source texture may have changed too, so drop the cached view of it
		if (quad_texture_view) {
			quad_texture_view->Release();
			quad_texture_view = nullptr;
		}
		quad_texture_view_src = nullptr;
		...
		if (srcDesc.SampleDesc.Count > 1) {
			...
			// Full source size (not the bounds-scaled size): ResolveSubresource requires the source and
			// destination to match, and the copy box in CopyToSwapchain reads the bounded region out of
			// the full-size resolve target.
			D3D11_TEXTURE2D_DESC resDesc = fullSrcDesc;
			resDesc.SampleDesc.Count = 1;
			resDesc.SampleDesc.Quality = 0;
			// We only ever resolve into slice 0 and copy back out of it, so a single slice is enough - and
			// keeping the source's ArraySize here would waste memory for array (stereo) render targets.
			resDesc.ArraySize = 1;
			resDesc.MiscFlags &= ~(UINT)D3D11_RESOURCE_MISC_GENERATE_MIPS;

			resolvedMSAATextures.resize(imageCount, nullptr);

			for (uint32_t i = 0; i < imageCount; i++) {
				OOVR_FAILED_DX_ABORT(device->CreateTexture2D(&resDesc, nullptr, &resolvedMSAATextures[i]));
			}
		}
...
void DX11Compositor::CopyToSwapchain(const vr::Texture_t* texture, const vr::VRTextureBounds_t* bounds, std::optional<XruEye> eye, vr::EVRSubmitFlags submitFlags)
{
	...
	if (bounds && bounds->vMin > bounds->vMax && oovr_global_configuration.InvertUsingShaders() && !swapchain_rtvs.empty()) {
		// Creating a view is not free, and the app hands us the same texture frame after frame - only
		// rebuild it when the source actually changes.
		if (quad_texture_view_src != src) {
			if (quad_texture_view) {
				quad_texture_view->Release();
				quad_texture_view = nullptr;
			}
			OOVR_FAILED_DX_ABORT(device->CreateShaderResourceView(src, nullptr, &quad_texture_view));
			quad_texture_view_src = src;
		}

		// Save every piece of context state we're about to overwrite. SteamVR's Submit leaves the
		// application's D3D11 state untouched, and engines that don't re-bind everything at the top of
		// each frame depend on that - previously we restored only the viewports, scissors, rasterizer
		// state and topology, and silently clobbered the render targets, shaders, blend state, sampler
		...
		// Unbind our SRV first: the source texture is very likely one of the app's render targets, and
		// leaving it bound as a shader resource would make the driver null out its RTV binding.
		...
		// All the ...Get... calls above return AddRef'd pointers, so they must be released. Failing to
		// release the rasterizer state here used to leak one reference per eye per frame.
		...
		if (srcDesc.SampleDesc.Count > 1) {
			// CheckChainCompatible rebuilds the chain (and these textures) when the sample count changes,
			// so this should always hold - but a mismatch here would be an out-of-bounds read, so check.
			OOVR_FALSE_ABORT(currentIndex < resolvedMSAATextures.size());

			// ResolveSubresource writes into subresource 0 of the resolve target (it only has one slice),
			// so the copy below must read from slice 0 too - not from arrayIndex, which would be an
			// entirely unwritten slice for the second eye of an array texture.
			context->ResolveSubresource(resolvedMSAATextures[currentIndex], 0, src, arrayIndex, srcDesc.Format);
...
#define CHECK(name, chainName)                  \
	...
	if (inputDesc.SampleDesc.Count != createInfoSampleCount) {
