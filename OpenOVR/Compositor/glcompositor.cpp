...
// glActiveTexture is GL 1.3 core, so unlike the extensions above it *is* declared in gl.h on Linux -
// only shadow it with a runtime-loaded pointer on Windows, where gl.h stops at GL 1.1.
#ifdef _WIN32
...
void GLBaseCompositor::CopyToSwapchain(const vr::Texture_t* texture, const vr::VRTextureBounds_t* bounds, std::optional<XruEye>, vr::EVRSubmitFlags)
{
	...
	// Save the app's bindings - the queries, blit and texture binding below would otherwise silently
	// redirect its subsequent rendering to our framebuffers.
	GLint prevDrawFramebuffer = 0, prevReadFramebuffer = 0, prevActiveTexture = 0, prevTexBinding = 0;
	...
	if (useBlit) {
		...
		// Restore the app's framebuffer bindings rather than resetting to 0 - it may well have had
		// its own FBO bound, and leaving ours attached would silently corrupt its rendering.
		glBindFramebuffer(GL_FRAMEBUFFER, prevDrawFramebuffer);
	...
	// Restore the app's texture unit and 2D texture binding
