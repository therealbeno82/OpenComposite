...
public:
	...
	/**
	 * Get the device's pose, locating it at the time the game wants it, rather than at the display
	 * time of the current frame.
	 *
	 * This is a request for an absolute prediction offset: predictedSecondsToPhotonsFromNow is
	 * seconds ahead of the *present* (negative = in the past), the same meaning as in OpenVR's
	 * GetDeviceToAbsoluteTrackingPose. Values < 0 are treated as "no prediction requested - use the
	 * current frame's display time", which keeps existing callers (and the frame-compositor path)
	 * working exactly as before.
	 *
	 * No default argument here: one would make plain 3-argument GetPose calls ambiguous.
	 */
	virtual void GetPose(
	    vr::ETrackingUniverseOrigin origin,
	    vr::TrackedDevicePose_t* pose,
	...
	{
		// Default implementation: ignore the prediction request and return the current pose. The XR
		// device classes override this to actually relocate the pose in time.
